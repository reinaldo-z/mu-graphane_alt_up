...
The spin density injection current $\dot{K}^{\mathrm{ab}}(\omega)$, where the
...
the sum only covers these states and since $\mu$ is real we have that
...
Considering that we have 2D structures, one of the options to analyze the spin
velocity (Eq. \eqref{eq:vab}) is fixing the spin along the $x$, $y$, or $z$
Cartesian coordinates. We define the magnitude of the spin velocity with spin
polarization along the $\mathrm{b}$ direction as
\begin{align}
...
Another possibility to analyze the spin velocity (Eq. \eqref{eq:vab}) is fixing
...
% 
For both cases, the most interesting case is that for which the spin is
perpendicular to the surface corresponding to $|\mathcal{V}_{\sigma^{\mathrm{z}}}(\omega,\alpha)|$.
...
an incoming energy beam from 0.0\,eV to 0.2\,eV which include the THz and the
Mid Infrared (MIR) radiation, where the absolute maximum of the
...
from evaluate Eq. \eqref{eq:gamma-ang}, and in the bottom frames the panels we
...
and the response direction angle are collinear corresponding to
...
\eqref{eq:gamma-par}) corresponding to
