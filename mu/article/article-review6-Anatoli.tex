...
Secondly,  we choose the spin-velocity direction within (OR along?) the surface
plane, and calculate its (which one) the spin-velocity injection speed and spin
orientation. This is done by changing the energy $\hbar\omega$ and polarization
angle $\alpha$  of the incoming  light. The results are calculated within a
full electronic band structure scheme using the Density Functional Theory (DFT)
in the Local Density Approximation (LDA). The maxima of the spin-velocity speed
are reached when $\hbar\omega=0.084$\,eV and $\alpha=35^\circ$ for the Up
structure, and $\hbar\omega=0.720$\,eV and $\alpha=150^\circ$ for the Alt
geometry. We find a speed of 668\,Km/s and 645\,Km/s for Up and Alt structures,
respectively, when the spin points perpendicularly to the surface. Also, the
response is maximized by fixing the spin-velocity direction along a high
...
pointing at 60$^\circ$ from the surface normal, for the Alt. These speed values
are significantly larger than those of bulk semiconductors, such as CdSe and
...
crucial (central), adding a new degree of freedom to conventional charge
...
the difficulties  in achieving (creating) measurable  the of spin current and
...
proven in [1]. In this paper, we present two 50\% hydrogenated graphene
noncentrosymmetric structures, both presenting a discernible band gap. The
first one, labelled as the Up structure (also known as graphone, see [2] and
Refs therein), shown in Fig. \ref{fig:up-struc}. Graphone has hydrogen atoms
bonded to the carbon layer only on the upper side of the structure, we consider
here the magnetic isomer of graphone, with the so-called “chair” structure. In
contrast,  the Alt structure, shown in Fig. \ref{fig:alt-struc}, has hydrogen
alternating on the upper and bottom sides of the carbon
sheet.\cite{zapataPSB2016}
...
structures, we do the analysis for two cases (not clear to me this sentence).
The first case is by fixing the spin of the electrons along vertical to the
surface direction (?)$z$, , with the velocity directed onto (is this parallel
or not?) the surface of the structure in the $xy$ plane. In the second case we
fix the spin velocity in the $x$ or $y$ direction and the resulting spin is
then directed outward of the $xy$ plane. 
% $xyz$. (Do we need to repeat “Cartesian” many times?)
The SVI is an optical effect that quantifies the velocity, at which a PSC moves
along the direction $\bf{a}$ with the $\bf{a}$ components to be determined,  
% ####???, $\mathrm{a}$ and 
with the spin of electron polarized along the direction $\bf{b}$. 
% $\mathrm{b}$. One-photon absorption of linearly
...
(?).\cite{bhatPRL05} If the crystalline structure of the material is
...
in this work.
%Although, this splitting is small [15,16], is required to obtain the correct
%results\cite{nastosPRB07}. The conduction bands in the noncentrosymmetric
%semiconductors are spin split by a small amount,[15,16] typically smaller than
%the energy width of the laser pulse, and so the pulse excites a coherent
%superposition of the two conduction bands. Even for very long pulses with
%narrow energy widths, dephasing effects lead to an energy width of the bands
%large enough that spin-split states can become quasi-degenerate.
...
Analyzing the SVI, Eq. \eqref{eq:vab-aw}, we define (calculate?) the magnitude
...
input for our calculations (the primitive hexagonal unit cell contains two C
and one H atoms, see Fig. 1). ADD EXTRA C atom }
...
\caption{$\mathcal{V}^{\mathrm{ab}}(\omega,\alpha)$ 
  ...
  Up structures, starting (going up?) at the energy gap of each structure. In
  the high energy regions, the values of
...
(confusing to me), and for the angle $\alpha$, at which the signal is maximized
, for the Up  and Alt structures, and for CdSe and GaAs bulk systems, shown for
comparison. As expected from Eq.~\eqref{eq:mu}, ${\cal
...
scale, red line, long dashes?), and ${\cal
V}_{\sigma^\mathrm{z}}(\omega,\alpha)$ (left scale, black solid line?), $vs$.
$\alpha$, for $\hbar\omega=0.084$\,eV, i.e. along the ridge shown in the 3D
plot. }
\label{fig:up-vsz-w1}
\end{figure}
...
From Eqns. \eqref{eq:polar-ang} and \eqref{eq:azimuthal-ang}, we determined the
polar, $\theta_{\mathrm{a}} (\omega,\alpha)$, and azimuthal,
...
$\hbar\omega$, for $\alpha=35^\circ$, and $\mathrm{a}=x$ and $\mathrm{a}=y$. }
...
$x$, and is almost parallel to the $xy$ plane in the first Cartesian quadrant
...
(\omega,\alpha)$ vs. $\hbar\omega$, in the range where there two local maxima
...
the spin is directed upward in the first Cartesian quadrant of the $xy$ plane
when it (who?) moves along either $x$ or $y$ directions.
...
% I believe that all the similar captions
...
% Change the captions accordingly the above
...
Cartesian quadrant when the spin velocity is directed along $y$. Finally, in
