...
for i in ${angles[@]}; do
    ...
    paste .temp1 | awk '{if($1 < 0) {printf("%07.1f\n", ($1*'"$deg"')+180) ;} else {printf("%07.1f\n"), $1*'"$deg"' } }' > .deg1
    paste "${inputkk[0]}""$i"  "${inputkk[1]}""$i"  .deg1 | awk '{printf("%02.5E    %02.6E    %03.0f    %02.6E    %02.6E    %05.1f\n"), $1, sqrt($2*$2 + $4*$4), '"$i"', $2, $4, $5 }' >> $outputkk
    echo  "" >> $outputkk
    
    paste "${inputsm[0]}""$i"  "${inputsm[1]}""$i" | awk '{if($4 != 0) {printf("%1.6E\n", atan2($4,$2)) ;} else if($4==0) {printf("%1.6E\n"), 0,0 } }' > .temp2
    paste .temp2 | awk '{if($1 < 0) {printf("%07.1f\n", ($1*'"$deg"')+180) ;} else {printf("%07.1f\n"), $1*'"$deg"' } }' > .deg2
    paste "${inputsm[0]}""$i"  "${inputsm[1]}""$i"  .deg2 | awk '{printf("%02.5E    %02.6E    %03.0f    %02.6E    %02.6E    %05.1f\n"), $1, sqrt($2*$2 + $4*$4), '"$i"', $2, $4, $5 }' >> $outputsm
    echo  "" >> $outputsm
done
}
